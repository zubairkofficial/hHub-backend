...
#         "status": "processing",
#     }
#     user_client_id = client_ids[0]
#     print(f"client_id = {user_client_id}")
#     # error
#     try:
...
#                 if exists:
#                     logger.info("Phone %s already exists in Laravel → skipping.", phone_number)
#                     active_sessions[session_id]["details"].append({
#                         "message": f"Skipped existing phone {phone_number}",
#                         "status": "skipped",
#                     })
#                     # ✅ do NOT add to phone_groups, do NOT process further
#                     continue
#                 if exists:
...
#             active_sessions[session_id]["status"] = "completed"
#             return {"status": "success", "processed_phone_numbers": 0}
#         print(f" we have total Phone group = {len(phone_groups)} and phone_group_number is = {json.dumps(phone_groups, indent=4)} of user_id = {user_id}")
...
#                 scores = await db.scoring_service.score_summary(analysis_summary,client_id=user_client_id)
#                 potential_score = (
...
def _derive_fullname(call: dict, transcript: Optional[str]) -> Optional[str]:
    # Prefer structured fields if your source provides them
    for k in ("caller_name", "customer_name", "name", "from_name"):
        if call.get(k):
            return str(call.get(k)).strip()
    ...
    # Try common patterns in the transcript
    patterns = [
        r"\b(?:This is|I am|I'm|My name is)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})\b",
    ]
    for pat in patterns:
...
ENUM_RECEIVE_DB_VALUE = "receive"  # match your DB enum spelling
...
def normalize_lead_for_laravel(lead: dict) -> dict:
    ...
    # type mapping
    t = ((lead.get("type") or "").strip().lower())
    if t not in ("receive", "miss"):
        t = "miss"
    if t == "receive":
        t = ENUM_RECEIVE_DB_VALUE

    # CallRail ID: DB column INT → only send a number, else NULL
    callrail_id = _int_or_none(lead.get("callrail_id"))
...
async def process_clients_background(client_ids: List[str], session_id: str, user_id: int):
    ...
    try:
        ...
        # 2) Group calls by phone (robust phone extraction) — SKIP if phone exists
        phone_groups: Dict[str, Dict[str, Any]] = {}
        ...
        async with httpx.AsyncClient(timeout=15.0) as httpc:
            ...
            for call in raw_calls:
                ...
                if exists:
                    ...
                    # do NOT add to phone_groups
                    continue

                logger.info("Phone %s not found in Laravel → queuing for processing.", phone_number)
                try:
                    await sendDirectCallRailApi(phone_number)
                except Exception as e:
                    logger.warning("sendDirectCallRailApi failed for %s: %s", phone_number, e)
...
async def sendDirectCallRailApi(phone: str):
    try:
        api_url = f"{apiurl}/api/save-callrail-data"
...
@router.post("/process-user-clients")
async def process_user_clients(request: Request, background_tasks: BackgroundTasks):
    """Process client scoring in background and return session ID for progress tracking"""
    try:
        body = await request.json()
        client_ids = body.get("client_ids", [])
        ...
        background_tasks.add_task(process_clients_background, client_ids, session_id, user_id)
        ...
        try:
            while session_id in active_sessions:
                ...
                progress_data = {
                    ...
                    "processed": session_data['processed'],
                    "total": session_data['total'],
                    "details": session_data['details'][-10:],
                    "percentage": round((session_data['processed'] / session_data['total']) * 100) if session_data['total'] > 0 else 0
                }
                yield f"data: {json.dumps(progress_data)}\n\n"
...
async def get_call_data():
    try:
        ...
        async def _phone_exists(httpc: httpx.AsyncClient, phone: str) -> bool:
            try:
                r = await httpc.get(f"{apiurl}/api/check-phone-number/{phone}")
                if r.status_code != 200:
                    return False
        ...
        # Build groups, skipping phones that already exist
        async with httpx.AsyncClient(timeout=15.0) as httpc:
            for call in call_data.get("data", []):
                phone_number = call.get("phone_number")
                recording_url = call.get("call_recording")
                ...
                if await _phone_exists(httpc, phone_number):
                    print(f"Skipping existing phone: {phone_number}")
                    continue
...
@router.get("/client-phone-score/")
async def get_client_phone_score(request: Request, client_ids: Optional[List[str]] = Query(None)):
    try:
        if not client_ids:
            query_params = request.query_params
            client_ids = []
...
@router.post("/prompt")
async def system_prompt_create_or_update(prompt: SystemPrompt):
    try:
        valid_roles = {"Super Admin", "Employee", "Project Manager", "Client"}
        if prompt.role_name and prompt.role_name not in valid_roles:
            raise HTTPException(status_code=400, detail=f"Invalid role_name. Must be one of {valid_roles}")
